...
int m = 100;
int n = 30;
float delay_factor = 1;
float motion_radius = 0.5;
float offset_factor = 0.007;
float radius = 100;
...
class WiggleLine {
    ...
    float x(float t) {
        return x + 100 * (float) noise.eval(seedX + motion_radius * cos(TWO_PI * t), motion_radius * sin(TWO_PI * t));
    }
    float y(float t) {
        return y + 100 * (float) noise.eval(seedY + motion_radius * cos(TWO_PI * t), motion_radius * sin(TWO_PI * t));
    }
}
...
void draw_() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < i; j++) {
            float distance = dist(array[i].x(t),array[i].y(t),array[j].x(t),array[j].y(t));
            float strokeColor = constrain(map(distance, 0, 150, 80, 40), 40, 80);
            float strokeValue = constrain(map(distance, 0, 100, 2, 0), 0, 2);
