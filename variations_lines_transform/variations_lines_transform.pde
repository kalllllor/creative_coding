...
void draw() { 
  if(go) {
    ...
    float s = alphaMax*pow(sin(PI*(iLines+t)/numberOfLines),2.0);
    float middle = width/2;
    float sizeW = 0.8 * middle;
    float sizeH = 0.7 * middle;
    float i = map(iLines + t, 0, numberOfLines, -3, 3);
    stroke(255, s);
    for(float j=-3.0; j<=3.0; j+=step){
      drawme(j, i);
    }
    iLines++;
    if(iLines == numberOfLines) {
      iFrame++;
      iLines = 0;
...
class Folds {
  ...
  PVector getFoldbyNo(int no, PVector p, float weight) {
      switch(no) {
          ...
          case 17: return sinusoidal(p,weight);
